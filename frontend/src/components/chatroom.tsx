...
export default function Chatroom({ messages, setMessages }: Props) {
  const cardBodyRef = useRef<HTMLElement | null>(null);
  ...
  const sendMessage = () => {
    ...
    setMessages((prevMessages: any) => [
      ...prevMessages,
      { message: newMessage, liveID, userID, name },
    ]);
    setNewMessage("");
  };
  ...
  return (
    <Card className=" w-full  max-h-[450px]">
      <CardHeader className="flex gap-3 justify-between text-lg font-bold">
        聊天室
      </CardHeader>
      <Divider />
      <div
        ref={cardBodyRef}
        className="p-4 flex flex-col min-h-[350px] max-h-[350px] overflow-y-scroll gap-4"
      >
