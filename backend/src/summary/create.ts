  ...
  app.post(
    ...
    async ({ profile, params, set }) => {
        ...
        try {
            ...
            const concat_doc_len = Math.floor(concat_doc.length*0.2) > 0 ? Math.floor(concat_doc.length*0.2) : 1
            ...
            console.warn(output, concat_doc_len,  Math.floor(concat_doc_len/5), output.length )

            const chain = loadSummarizationChain(new OpenAI({ temperature: 0 }), { type: "map_reduce" });
