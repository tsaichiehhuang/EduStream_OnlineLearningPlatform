        ...
        {(onClose) => (
          <>
            <ModalHeader className="flex flex-col gap-1">
              上傳檔案：{data.homework.title}
            </ModalHeader>
            <ModalBody>
              <p className="text-sm">詳細說明</p>
              <p className="text-sm text-gray-500">
                {data.homework.description || "無"}
              </p>
              {userRole !== "student" && (
                <>
                  {" "}
                  ...
                  {hwData.length === 0 && (
                    <div className="text-sm text-gray-500">尚無繳交資料</div>
                  )}
                  {hwData.map((item: any) => (
                    <div key={item.id}>
              ...
              {userRole === "student" && (
                <>
                  ...
                  {data.homework.done && !selectedFile && (
                    <div className="my-2 text-mainOrange">
                      已選擇: {data.homework.name}
                    </div>
                  )}
                  {selectedFile && (
                    <div className="my-2 text-mainOrange">
    ...
    <>
      <div className="flex flex-row justify-between">
        <Card
          onPress={onOpen}
          isPressable
          className="p-1 md:w-fit hover:bg-neutral-50 "
          shadow="sm"
        >
        ...
        {editMode && (
          <div className="gap-1 flex flex-row items-start">
            <Delete
              status="submit"
              id={data.hwId}
              onClick={Cookies.set("status", "submit")}
            />
